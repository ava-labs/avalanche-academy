...
In the previous sections of this chapter we manually deployed the Interchain Token Transfer contracts. In this section we will use the Avalanche CLI to deploy the contracts and transfer the tokens.
...
Now we can check the balances of both the ERC20 Home token and the Remote one. 
