...
```bash
cast send $ERC20_CONTRACT_SUBNET --private-key $PK "approve(address,uint256)" $ERC20_RECEIVER_SUBNET 20ether --rpc-url mysubnet
```
...
```bash
cast send $ERC20_RECEIVER_SUBNET --private-key $PK "receiveTokens(address,uint256)" $EWOQ 2ether --rpc-url mysubnet
```
...
```bash
cast call $ERC20_CONTRACT_SUBNET "balanceOf(address)(uint256)" $ERC20_RECEIVER_SUBNET --rpc-url mysubnet
```