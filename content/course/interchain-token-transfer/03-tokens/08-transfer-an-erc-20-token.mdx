...
```bash
avalanche key list -l --subnets myblockchain --tokens $ERC20_CONTRACT_SUBNET,native  --keys myAddress,ewoq
```
...
```bash
cast send $ERC20_CONTRACT_SUBNET --value 0 --private-key $PK "transfer(address,uint256)" $MYADDRESS 200ether --rpc-url myblockchain
```
