...
To facilitate more complex interactions, such as those involving smart contracts, the ERC-20 standard includes the approve and transferFrom functions. The approve function allows a token holder to authorize another address (often a smart contract) to spend a specified amount of tokens on their behalf. This sets an allowance, which is the maximum amount the authorized address can withdraw. Subsequently, the transferFrom function is called by the authorized address to transfer tokens from the holder to another address or contract, up to the approved limit. This two-step mechanism ensures greater security and control, as it prevents unauthorized transfers and allows for more intricate transaction flows within decentralized applications. By leveraging these functions, developers can design sophisticated token interactions, such as automated trading, token swaps, and decentralized finance (DeFi) protocols.
...
These two are needed for a ERC20 transfer Smart contract
...
## Transfer ERC20 between accounts

To transfer ERC20 between accounts you need to modify the balance registry from 
```
```
