...
```
Here we define the internal API methods for our backend. We have two methods that we will be working with in this section: `getBlockHeight` and `listErc20Balances`. We create both of these methods internally, then forward the request to the Glacier Data API. We then return the result to the client.
</Step>
<Step>
```tsx title="src/app/api/balance/route.ts"
...
```
In the next section, we will implement the `listErc20Balances` and `getBlockHeight` function to call the Glacier Data API through the AvaCloudSDK.
</Step>
</Steps>
</Step> 
</Steps> 