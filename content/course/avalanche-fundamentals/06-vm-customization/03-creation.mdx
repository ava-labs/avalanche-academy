...
General Purpose VMs introduce another layer - commonly know as Smart Contracts or Programs - that can be submitted to the VM and enable user of the chain to interact with these. This way it allows its users to introduce their own logic to the VM. The most common General Purpose VM is the Ethereum Virtual Machine (EVM). However, some Subnets creators might want to create a VM that utilizes another language, such as Move, instead of Solidity as it’s smart contract language.

![](/course/avalanche-fundamentals/35.png)
