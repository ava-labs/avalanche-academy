...
```bash
forge create --rpc-url myblockchain --private-key $PK src/interchain-messaging/4-incentivize-relayer/senderWithFees.sol:SenderWithFeesOnCChain
```
...
```bash
forge create --rpc-url myblockchain --private-key $PK src/interchain-messaging/4-incentivize-relayer/receiverWithFees.sol:receiverWithFeesOnCChain
```
