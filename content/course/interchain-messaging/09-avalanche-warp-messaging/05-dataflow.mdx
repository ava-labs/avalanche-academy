...
3. An AWM Relayer relays the message to the destination Chain. It periodically checks the source Subnet for outgoing messages and delivers these by calling the Interchain Messaging contract on the destination Subnet
4. The Interchain Messaging contract on the destination chain interacts with AWM to verify the signature the message and whether it has been signed by a sufficiently large stake share of the source Subnet's validator set.
