...
- **Query the P-Chain:** The verifying validators each receive the validator set of the source Avalanche L1 with its stake weights and BLS Public Keys
- **Verify Stake Weight:** First, the verifying validators are checking if the combined stake weight of the validators that have signed the Warp message is large enough to accept the message (e.g. validators representing 50% of the total stake have signed the message). The required stake weight may vary from chain to chain. An Avalanche L1 may require 50% for chain A and 90% for chain B. If the stake weight is insufficient the message is rejected.
