...
The following contract is located inside `src/interchain-messaging/0a-send-receive` directory. Read through the contract below and and understand what is happening:

```solidity title="src/interchain-messaging/0a-send-receive/receiverOnSubnet.sol"
...
```bash
forge create --rpc-url myblockchain --private-key $PK src/interchain-messaging/0a-send-receive/receiverOnSubnet.sol:ReceiverOnSubnet
```
