...
Each Avalanche network has its own Primary Network, which comes with a Contract (C), Platform (P), and Exchange (X) chain. Note, that the naming "Primary Network" isn't actually very accurate, since it's a special Subnet, not its own network. 
...
- Command Line: Commands can be issued using the command line, e.g. `avalanche network clean`