...
## Adding Network in Foundry

This is the most recommended way to interact with your blockchain. You can easily use foundry-based commands to deploy contracts and interact with different wallets and functionalities. 

In order to start using foundry commands, you'll need to add the RPC URL of your blockchain in `foundry.toml` file.

For example, in our Codespace from the [Avalanche Starter Kit](https://github.com/ava-labs/avalanche-starter-kit), we named our blockchain `mysubnet`. Here is how the foundry configuration looks like in our case:

```toml title="foundry.toml"
mysubnet = "http://localhost:9650/ext/bc/mysubnet/rpc"
```

If you created your subnet with a different name, make sure to add the RPC URL in the `foundry.toml` file.

## Adding Network in Core
