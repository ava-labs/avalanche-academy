...
To create a Custom Network configuration run the following commands:

```
avalanche subnet create <subnetName>
```

You will be prompted a Wizard to set the custom network config. For now, follow the default options where possible.
...
Now that we created our custom network config, we need to that will host that network. The following command will launch 5 nodes running locally:
...
If you don't start your network manually it will be automatically started when you deploy your first custom blockchain network.
...
This output includes the endpoints of your 5 nodes where you can connect to interact with your custom network. You're running a Subnet-EVM, this means you can use those RPC URLs to connect to EVM compatible wallet or tools. 
