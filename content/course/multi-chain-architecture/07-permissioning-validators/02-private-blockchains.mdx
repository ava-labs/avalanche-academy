...
The data of Avalanche Subnets, by default, are publicly available. This means that every node can sync and listen to ongoing transactions/blocks in Subnets, even if they're not validating the listened Subnet. Nodes may do this for indexing purposes or just having access to the current state of the blockchain without relying on third-parties.

Subnet validators can choose not to publish data from their blockchains. If a node sets validatorOnly to true, the node exchanges messages only with this Subnet's validators. Other peers won't be able to learn contents of this Subnet from this node.