...
AWM allows Subnets to communicate with one another via authenticated messages by providing signing and verification primitives in AvalancheGo. These are used by the blockchain VMs to sign outgoing messages and verify incoming messages. Any Virtual Machine (VM) on Avalanche can integrate AWM to send and receive messages across Avalanche Subnets. 

Therefore, AWM is not EVM specific, but rather a general cross-Subnet communication protocol.
