---
title: 🗺️ Running a Relayer
description: Learn about the basics of Avalanche.
updated: 2024-05-31
authors: [ashucoder9]
---

As you know now Teleporter requires a running relayer to deliver the warp messages underlying it. So far we just assumed a relayer is running and in fact Avalanche CLI automatically configures and runs a relayer for you if you deploy your Subnet on a local network. However, in a production environment, you will either need to run a relayer yourself or have someone else run it for you.

![AWM Relayer data flow](/course-images/teleporter/teleporter-source-destination-with-relayer.png)

In this section you will learn how to run a relayer for Avalanche Warp Messaging. As discussed in the section on AWM, the relayer is responsible for checking for outgoing warp messages on the source Subnet, aggregate the signatures and deliver it with a transaction on the destination Subnet.
...
- **Send Message:** Next you will issue a transaction triggering a cross-Subnet message, so we can observe how the message is delivered by the relayer.
