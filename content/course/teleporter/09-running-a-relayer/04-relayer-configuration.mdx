...
For each destination Subnet, the relayer has the following config parameter:

- **subnet-id:** The ID of the Subnet that the destination blockchain is part of. 
- **blockchain-id:** The blockchain ID of the destination blockchain. This is the Blockchain ID of Fuji's Dispatch Subnet.
- **vm:** A string specifying the virtual machine of the destination Subnet's blockchain. Currently, only the EVM is supported, but this field has been added in anticipation of communication between blockchains powered by different virtual machines in the future.
...
- **account-private-key:** A private key for a wallet that holds gas tokens on the destination Subnet. This is required so the relayer can sign the transaction that delivers the warp message.
