...
A relayer is configured to relayer messages between certain source and destination Avalanche L1s. Therefore, the configuration consists of three parts:
- **General Config:** Configuration independent of the source and destination Avalanche L1s concerning the network as a whole
...
Next is the configuration for our source blockchain. This is the configuration of the blockchain where messages will be initiated and picked up. The relayer will aggregate the signatures of the validators of that Avalanche L1. 
...
- **subnet-id:** The Avalanche L1 ID of the Avalanche L1 that the source blockchain is part of. In this example this is the Avalanche L1 ID of the Primary Network.
- **blockchain-id:** The blockchain ID of the source. In this example this is the blockchain ID of Fuji's C-Chain.
- **vm:** A string specifying the virtual machine of the destination Avalanche L1's blockchain. Currently, only the EVM is supported, but this field has been added in anticipation of communication between blockchains powered by different virtual machines in the future.
...
- **settings > reward-address:** The address that will be rewarded if an Avalanche L1 is incentivizing relayers to send messages on it's behalf. This is the address of the wallet you will create for this relayer. 
</div>
...
For each destination Avalanche L1, the relayer has the following config parameter:

- **subnet-id:** The ID of the Avalanche L1 that the destination blockchain is part of. 
- **blockchain-id:** The blockchain ID of the destination blockchain. This is the Blockchain ID of Fuji's Dispatch Avalanche L1.
- **vm:** A string specifying the virtual machine of the destination Avalanche L1's blockchain. Currently, only the EVM is supported, but this field has been added in anticipation of communication between blockchains powered by different virtual machines in the future.
- **rpc-endpoint:** The RPC endpoint of the destination subnet's API node. Used in favor of api-node-host, api-node-port, and encrypt-connection when constructing the endpoint.
...
- **account-private-key:** A private key for a wallet that holds gas tokens on the destination Avalanche L1. This is required so the relayer can sign the transaction that delivers the warp message.
