...
description: Adapt the receiver contract to also send messages back
updated: 2024-05-31
...
Therefore, we need to change the receiver contract to be able to send a message back. We will need to instantiate a `TeleporterMessenger` and call the the `sendCrossChainMessage()` function. 

```solidity title="src/1-send-roundtrip/receiverOnAvalanche L1.sol"
