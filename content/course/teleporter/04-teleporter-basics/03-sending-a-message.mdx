...
description: Learn to send messages with Teleporter.
updated: 2024-05-31
...
The dApp on the Source Avalanche L1 has to call the `sendCrossChainMessage` function of the Teleporter contract. The Teleporter contract implements the `ITeleporterMessenge`r` interface below. Note that the dApp itself does not have to implement the interface.
