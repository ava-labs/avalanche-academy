...
```
`fetchRecentTransactions` is a function that will make a call to our backend to get the user's ERC-721 token balances. It will call the `getRecentTransactions` method on our backend with the user's address. It will then return the balances as an array of `NativeTransaction` objects.
...
`fetchRecentBlocks` is a function that will make a call to our backend to get the user's ERC-1155 token balances. It will call the `getRecentBlocks` method on our backend with the user's address. It will then return the balances as an array of `EvmBlock` objects.
